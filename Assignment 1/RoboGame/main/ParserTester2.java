...
public class ParserTester2 {
    ...
    private static final String[][][] programs = new String[][][]{{//STAGE 0
            {"VALID", "move;", "move action"}, {"VALID", "turnL;", "turnL action"}, {"VALID", "turnR;", "turnR action"}, {"VALID", "takeFuel;", "takeFuel action"}, {"VALID", "wait;", "wait action"}, {"VALID", "move; turnL; turnR; move; takeFuel; ", "sequence of actions"}, {"VALID", "loop{move ;}", "loop with a Block with one action"}, {"VALID", "loop{move; wait; turnL; turnR;}", "loop with a Block with four actions"}, {"VALID", "loop{move; loop{turnL;}}", "nested loop"}, {"VALID", "move; turnL; turnR; move; takeFuel; loop{move; turnR; wait;}", "all stage 0 elements"},

            {"INVALID", "move; turnR move;", "missing ;"}, {"INVALID", "move; turnR: move;", ": instead of ;"}, {"INVALID", "move; turnL; turnRight; move;", "invalid action turnRight"}, {"INVALID", "loop{}", "Block in a loop with no statements"}, {"INVALID", "{move;}", "Block not inside a loop"}, {"INVALID", "loop{move; turnL;", "Block with no close }"}, {"INVALID", "loop{move; loop{turnL;}", "nested loop with one missing close } on block "}, {"INVALID", "loop{move; loop{turnL;", "nested loop with two missing close } on blocks "}, {"INVALID", "loop{move; turnL;}}", "Block with extra close }"},}, {//STAGE 1:
            {"VALID", "while(eq(fuelLeft, 2)) { wait; }", "while and condition using eq and  fuelLeft"}, {"VALID", "if(lt(oppLR, 2)) { wait; }", "if with condition using lt and oppLR"}, {"VALID", "if(gt(oppFB, 2)) { move; }", "if with condition using gt and oppFB"}, {"VALID", "if(eq(numBarrels, 1)) {turnL;}", "if with condition using eq and numbBarrels"}, {"VALID", "while(lt(barrelLR, 1)) {turnR;}", "while with condition using lt and barrelLR"}, {"VALID", "while(gt(barrelFB, 1)) {wait;}", "while with condition using gt and barrelFB"}, {"VALID", "while(eq(wallDist, 0)) {turnL; wait;}", "while with condition using eq and wallDis"},

            {"VALID", "while(gt(wallDist, 0)) {while(eq(fuelLeft, 4)) {turnL;}}", "while with nested while"}, {"VALID", "while(gt(wallDist, 0)) {if(eq(fuelLeft, 4)) {turnL;}}", "while with nested if"}, {"VALID", "if(gt(wallDist, 0)) {if(eq(fuelLeft, 4)) {turnL;}}", "if with nested if"}, {"VALID", "if(gt(wallDist, 0)) {while(eq(fuelLeft, 4)) {turnL;}}", "if with nested while"}, {"VALID", "move; while(gt(wallDist, 0)) {turnL;} if(eq(fuelLeft, 4)) {turnL;} wait;", "sequence of 4 statements, including an if an a while"},

            {"INVALID", "while{move;}", "while needs a condition"}, {"INVALID", "if{move;}", "if needs a condition"}, {"INVALID", "while(){move;}", "while can't have an empty condition"}, {"INVALID", "if(){move;}", "if can't have an empty condition"}, {"INVALID", "if(eq(fuelLeft, 1) {move;}", "Condition in if must have closing )"}, {"INVALID", "if eq(fuelLeft, 1) {move;}", "Condition in if must have opening )"}, {"INVALID", "while(eq(fuelLeft, 1) {move;}", "Condition in while must have closing )"}, {"INVALID", "while eq(fuelLeft, 1) {move;}", "Condition in while must have opening )"},

            {"INVALID", "while(eq(fuelLeft, 2) move;", "while must have a block, not a statement."}, {"INVALID", "if(eq(fuelLeft, 2) move;", "if must have a block, not a statement."}, {"INVALID", "while(eq(fuelLeft, 1)){}", "block in a while must have at least one statement"}, {"INVALID", "if(eq(fuelLeft, 1)){}", "block in an if must have at least one statement"},

            {"INVALID", "if(eq(fuelLeft, 2)) move;", "if must have a block, "}, {"INVALID", "if(shieldOn){shieldOff;}", "can't have an action as a boolean"}, {"INVALID", "if(gt(turnL, 1)) {move;}", "can't have an action as a sensor."}, {"INVALID", "loop(gt(turnL, 1)){move; turnL;}", "loop cannot have a condition"},}, {//STAGE 2

            {"VALID", "move(3);", "move with number argument"}, {"VALID", "move(fuelLeft);", "move with sensor argument"}, {"VALID", "move(add(fuelLeft,2));", "move with add argument"}, {"VALID", "move(mul(oppLR,2));", "move with mul argument"}, {"VALID", "wait(sub(oppFB,2));", "wait with sub argument"}, {"VALID", "wait(div(oppLR,2));", "wait with div argument"}, {"VALID", "wait(div(add(3, 5), sub(mul(oppLR,2),sub(5, 6))));", "wait with complex nested expression"}, {"VALID", "if (lt(add(3,4), sub(10,2))) { wait; } else {move;}", "lt on expressions, if with else"}, {"VALID", "if (gt(mul(3,4), div(100,2))) { wait; } else {move;}", "gt on expressions, if with else"}, {"VALID", "while (eq(mul(3,add(1, 4)), 10)) { wait;}", "eq with nested expression, "}, {"VALID", "if (and(lt(3,4),gt(10,2))) { wait; } else {move;}", "condition with and"}, {"VALID", "if (or(lt(3,4),gt(10,2))) { wait; } else {move;}", "condition with or"}, {"VALID", "if (not(lt(4,3))) { wait; } else {move;}", "condition with not"}, {"VALID", "if (or(and(lt(3,4),gt(10,2)), not(not(lt(4,3))))) { wait; } else {move;}", "nested ands, ors, nots"}, {"VALID", "if (eq(oppLR,3)) { wait; } else {move;}", "wait"}, {"VALID", "if (eq(barrelFB,3)) { wait; } else {move;}", "move"}, {"VALID", "if (eq(3,oppLR)) { wait; } else {move;}", "wait"}, {"VALID", "if (eq(3,barrelFB)) { wait; } else {move;}", "move"},
        ...
        for (int stage = 0; stage < 4; stage++) {
            ...
            for (String[] test : programs[stage]) {
                ...
                try {
                    ...
                    if (valid) {
                        if (ast != null) {
                            System.out.println("OK, valid program: " + printedForm);
                        } else {
                            System.out.println("BAD, failed to generate tree for valid program");
                        }
                    } else {
                        if (ast != null) {
                            System.out.println("BAD, program is invalid, parser gave: " + printedForm);
                        } else {
                            System.out.println("???, program is invalid, parser did not throw exception but did not build a tree");
                        }
                    ...
                    if (valid) {
                        System.out.println("BAD, threw exception for a valid program");
                    } else {
                        System.out.println("OK, parser correctly threw an exception");
                    }
                } catch (Exception e) {
